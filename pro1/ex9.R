...
# 레코드 추가
iquery <- "insert into sangdata values(5, '핸드크림',34,5000)"
dbSendUpdate(conn, iquery)

df <- data.frame(code=6,sang='립밤',su=12,dan=2000)
df
dbSendUpdate(conn, "insert into sangdata values(?,?,?,?)", df$code, df$sang, df$su, df$dan)
dbGetQuery(conn, "select * from sangdata")

# 레코드 수정
uquery <- "update sangdata set sang='현성운동화' where code=11"
...
# 참고
paste("Hi", "Everybody") # "Hi Everybody"  공백을 자동으로 만들어서 출력
paste0("Hi", "Everybody") # "HiEverybody" 공백이 없이 붙어서 출력
...
func <- function(conn, table, df){
  batch <- apply(df, 1, FUN = function(x) paste0("'", trimws(x), "'", collapse=",")) %>%   # trimws = 앞뒤의 공백을 없앤다.
                    paste0("(",.,")", collapse = ",\n")
  # print(batch)
